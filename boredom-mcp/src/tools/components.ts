...
import type { Bridge, ResponseMeta, ExportedComponent } from "../types.js"
...
export function registerComponentTools(server: McpServer, bridge: Bridge): void {
   ...
   */
  server.tool(
    "boredom_define_component",
    "Define a new boreDOM web component at runtime. Creates a custom element that can be used in HTML.",
    {
   ...
   */
  server.tool(
    "boredom_get_component",
    "Get details of a running component instance including its current state, refs, slots, and template.",
    {
    ...
    async ({ selector }) => {
      ...
      try {
        const result = await bridge.call("exportComponent", { selector }) as ExportedComponent | null
   ...
   */
  server.tool(
    "boredom_operate_component",
    "Get live access to a component's internals for inspection. Returns refs, slots, and current state.",
    {
