 ...
 */
export function registerCodeTools(server, bridge) {
    /**
     * Validate code before execution
     * Catches syntax errors, undefined references, and type issues
     */
    server.tool("boredom_validate_code", "Validate JavaScript code before execution. Catches syntax errors, undefined references, and type issues. Use this before boredom_apply_code to ensure code is safe.", {
        ...
        try {
            const result = await bridge.call("validate", { code });
            const meta = {
     ...
     */
    server.tool("boredom_apply_code", "Apply JavaScript code to the running boreDOM application. Automatically validates first and rolls back on error. Code can reference 'state' and 'boreDOM' globals.", {
        ...
        try {
            ...
            if (!skipValidation) {
                const validation = await bridge.call("validate", { code });
                if (!validation.valid) {
                    return {
                        content: [{
                                type: "text",
            ...
            // Apply the code
            const result = await bridge.call("apply", { code });
            const meta = {
     ...
     */
    server.tool("boredom_apply_batch", "Apply multiple code blocks atomically. All blocks succeed or all rollback. Use for multi-step changes that should be applied together.", {
        ...
        try {
            const result = await bridge.call("applyBatch", { codeBlocks });
            const meta = {
                suggestedNextTool: result.success ? "boredom_get_context" : "boredom_get_focus",
                hint: result.success
