...
function analyzeComponentModule(code) {
  ...
  const metadata = parseMetadataExport(exports.metadata, context, issues, fatalIssues);
  const style = evaluateStringExport(exports.style, context);
  const template = evaluateStringExport(exports.template, context);
  const logicSource = extractLogicSource(exports.logic, code, bindings);
  if (style === null) {
